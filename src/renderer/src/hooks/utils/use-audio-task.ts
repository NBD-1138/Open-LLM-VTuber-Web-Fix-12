...
import { emitAutomationAudioCompleted } from "@/utils/automation-audio-events";
import { applyStoredAudioOutputDevice } from "@/utils/audio-device-settings";
import { loadTtsPlaybackVolume } from "@/hooks/sidebar/setting/tts-playback-settings";
    ...
    new Promise((resolve) => {
      ...
      try {
        ...
        if (audioBase64) {
          ...
          audio.volume = loadTtsPlaybackVolume();
          ...
          const cleanup = () => {
            ...
            if (automationRequestId) {
              emitAutomationAudioCompleted(automationRequestId);
            }
            if (!isFinished) {
              isFinished = true;
              resolve();
          ...
          void applyStoredAudioOutputDevice(audio)
            .catch((error) => {
              console.warn(
                "[AudioDevices] Unable to route TTS playback to the selected output device",
                error,
              );
            })
            .finally(() => {
        ...
        if (options.automationRequestId) {
          emitAutomationAudioCompleted(options.automationRequestId);
        }
        resolve();
      }
    });

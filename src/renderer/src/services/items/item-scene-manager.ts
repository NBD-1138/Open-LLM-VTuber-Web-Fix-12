/* eslint-disable class-methods-use-this, no-param-reassign, no-restricted-syntax, no-continue, no-nested-ternary, no-useless-return, no-underscore-dangle */
...
const DISABLE_ITEMS_LAYER = false;
...
const ensureCubismRuntime = (): Promise<void> => {
  ...
  if (globalAny.Live2DCubismCore) {
    if (!globalAny.__Live2DCubismCoreReady) {
      globalAny.Live2DCubismCore.RESERVED_FUNCTION_POINTERS = Math.max(
        Number(globalAny.Live2DCubismCore.RESERVED_FUNCTION_POINTERS) || 0,
        4096,
      );
      globalAny.Live2DCubismCore.ALLOW_TABLE_GROWTH = 1;
      globalAny.__Live2DCubismCoreReady = true;
    }
    return Promise.resolve();
  }
  console.warn(
    "[ItemSceneManager] Live2DCubismCore is not available yet. Ensure index.html includes live2dcubismcore.js.",
  );
  return Promise.resolve();
};
...
export class ItemSceneManager {
  ...
  public async initialize(canvas: HTMLCanvasElement): Promise<void> {
    ...
    this.avatarContainer.zIndex = 0; // model baseline at 0; items can go negative to appear behind
    this.avatarContainer.eventMode = "dynamic";
    this.avatarContainer.interactiveChildren = true;
    this.avatarContainer.hitArea = this.app.screen;
  ...
  public async loadAvatarModel(modelJsonUrl: string): Promise<Live2DModel | null> {
    ...
    try {
      ...
      model.cursor = "move";
      model.zIndex = 0; // baseline at 0 within avatar container
    ...
    if (!(model as any).hitArea && typeof model.getLocalBounds === "function") {
      const bounds = model.getLocalBounds();
      const width = bounds.width || 1;
      const height = bounds.height || 1;
      (model as any).hitArea = new PIXI.Rectangle(bounds.x, bounds.y, width, height);
    }

      this.avatarModel = model;
      this.avatarAttachTargets = this.discoverAttachTargets(model);
      this.bindAvatarInteractions(displayModel);
  ...
  private async createDisplayObject(
    ...
    model.eventMode = "dynamic";
    model.cursor = "move";
    // Avatar model z-index relative to its container.
    model.zIndex = 0;
    ensureInteractiveMethod(model);
    ...
    if (!avatarContainer) {
      throw new Error("Avatar container is not initialized");
    }
    avatarContainer.addChild(displayModel);
    avatarContainer.sortChildren();
    if (!(model as any).hitArea && typeof model.getBounds === "function") {
  ...
  private bindInteractions(sceneItem: SceneItemInternal): void {
    ...
    // Ensure hit testing is enabled for Live2D models
    (display as any).interactive = true;
    (display as any).buttonMode = false;
  }
  ...
  private bindAvatarInteractions(modelDisplay: SceneDisplayObject): void {
    modelDisplay.eventMode = "dynamic";
    (modelDisplay as any).interactive = true;
    (modelDisplay as any).buttonMode = true;
    modelDisplay.cursor = "move";
    const target = this.avatarContainer ?? modelDisplay;
    if (target) {
      target.eventMode = "dynamic";
      target.cursor = "move";
      target.hitArea = target.hitArea ?? this.app?.screen;
    }
    modelDisplay.on("wheel", (event: PIXI.FederatedWheelEvent) => {
      if (!this.avatarContainer) return;
      const factor = 1 - event.deltaY * 0.0015;
      const next = clampScale(this.avatarContainer.scale.x * factor);
      this.avatarContainer.scale.set(next);
      this.avatarScale = next;
      this.items.forEach((item) => {
        if (!item.pinnedToAvatar || !item.localPosition) return;
        const world = this.localToWorld(item.localPosition);
        item.worldPosition = world;
        const localScale = item.localScale ?? item.pixiRef.scale.x;
        item.worldScale = localScale * this.avatarScale;
    ...
    modelDisplay.on("pointerdown", (event: PIXI.FederatedPointerEvent) => {
      const parent = modelDisplay.parent;
      if (!parent || !this.avatarContainer) return;
      const global = event.getLocalPosition(parent);
      const offsetX = this.avatarContainer.position.x - global.x;
      const offsetY = this.avatarContainer.position.y - global.y;
      this.avatarDrag = {
        pointerId: event.pointerId,
        offsetX,
        offsetY,
      };
      modelDisplay.on("pointermove", this.handleAvatarPointerMove, this);
      modelDisplay.on("pointerup", this.handleAvatarPointerUp, this);
      modelDisplay.on("pointerupoutside", this.handleAvatarPointerUp, this);
      modelDisplay.on("pointercancel", this.handleAvatarPointerUp, this);
    });
  }

  private handleAvatarPointerMove(event: PIXI.FederatedPointerEvent): void {
    if (!this.avatarDrag || !this.avatarContainer) return;
    const parent = (event.currentTarget as SceneDisplayObject).parent;
    if (!parent) return;
    const global = event.getLocalPosition(parent);
    const nextPos = {
      x: global.x + this.avatarDrag.offsetX,
      y: global.y + this.avatarDrag.offsetY,
    };
    this.avatarContainer.position.copyFrom(nextPos);
    this.items.forEach((item) => {
      if (!item.pinnedToAvatar || !item.localPosition) return;
      const world = this.localToWorld(item.localPosition);
      item.worldPosition = world;
  ...
  private handleAvatarPointerUp(event: PIXI.FederatedPointerEvent): void {
    const display = event.currentTarget as SceneDisplayObject;
    display.removeListener("pointermove", this.handleAvatarPointerMove, this);
    display.removeListener("pointerup", this.handleAvatarPointerUp, this);
    display.removeListener("pointerupoutside", this.handleAvatarPointerUp, this);
    display.removeListener("pointercancel", this.handleAvatarPointerUp, this);
  ...
  public async spawnItem(
    ...
    if (DISABLE_ITEMS_LAYER) {
      ...
      const stub: SceneItemInternal = {
        itemId,
        assetKey,
        kind: catalogAsset.kind,
        pixiRef: new PIXI.Container() as SceneDisplayObject,
        visible: false,
        pinnedToAvatar: false,
        worldPosition: { x: 0, y: 0 },
        worldScale: DEFAULT_ITEM_SCALE,
        worldZIndex: 0,
        localPosition: undefined,
        localScale: undefined,
        localZIndex: undefined,
      ...
      this.items.set(itemId, stub);
      useSceneItemsStore.getState().addActiveItem(stub);
      return stub;
    }
